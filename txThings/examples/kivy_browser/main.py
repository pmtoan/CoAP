...
class MainScreen(Screen):
    ...
    def send_request(self, result, card, request):
        ...
        def printResponse(self, response):
            card.response_payload.text += response.payload
        def iterate_requests():
            try:
                ...
                scheme, netloc, path, query, fragment = urlsplit(self.address_bar.text)
                coap_host, coap_port = netloc.split(':')
                card.response_payload.text += "\nRequest to %s:%s" % (host, port)
                r, source = path.split('/')
                request_copy = copy.deepcopy(request)
                # if scheme != "coap":
                #     card.response_payload.text += 'Error: URI scheme should be "coap"'
                # request_copy.remote = (str(host), port)
                # if path != "" and path != "/":
                #     path = path.lstrip("/")
                ...
                request_copy.opt.uri_path = (source,)
                request_copy.opt.observe = 0
                request_copy.remote = (coap_host, int(coap_port))
                d = None
                ...
                # with open("C:\\Python\\txThingsData.hat", 'r') as r:
                #     line = r.readline()
                #     card.response_payload.text += "\n" + line
                pending.append(d)
